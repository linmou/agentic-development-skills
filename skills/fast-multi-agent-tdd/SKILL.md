...
6. <!-- initial-pre-red-snapshot-publication --> Only after that pass, run `python scripts/tdd_snapshot.py create --feature <feature> --phase pre_red --roles audits/<feature>_role_receipt.json --include audits/<feature>_request_map.md --include audits/<feature>_scope_red.json` and verify its returned ref and commit. Red unlocks only when both the monitor pass and snapshot publication succeed.
...
Read `Overview`, `Trigger Conditions`, `Core Rules`, and Workflow sections 0-1 in [references/workflow.md](references/workflow.md), plus [references/phase_contracts.md](references/phase_contracts.md). Keep only executable behavior inside TDD, define the smallest end-to-end slice and done conditions, classify `compact|full`, preflight path semantics, and prepare the request map. <!-- tdd-shape-eligibility-gate --> Before step 1 saves the map, confirm the slice is TDD-shaped: red needs a test-like artifact to edit and Green needs a production-side artifact to change, distinct from every planned red test path. When the whole delta is test-side, or the fix is the test's own data or expectation, stop here and continue outside this skill as test maintenance. Then execute the mandatory start gate above; step 1 saves the map and activates TDD.
...
- Snapshots and guards respect Git ignore rules for ordinary code and data, and force the gate artifacts named by `--roles`, `--provenance`, and `--include` into the snapshot index, so a repository rule covering the evidence directory never needs an ignore negation or a global force-add. Name the request map and each scope artifact in `--include` at every create. Reviewer-owned audit JSON files stay untracked, are dropped from the snapshot tree, are hash-checked from live bytes, and are never reported as phase changes by the guard; that naming rule is shared by both tools and requires the scope artifact's `feature` to match the role receipt. Ignored unnamed data is outside snapshot and guard coverage. Read the coverage contract in [references/phase_contracts.md](references/phase_contracts.md) before snapshotting.
