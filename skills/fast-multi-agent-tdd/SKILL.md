...
- Snapshots and guards respect Git ignore rules. Keep required phase files and in-repository evidence tracked in the baseline or non-ignored; reviewer-owned audit JSON files stay untracked, the loaded `review-with-multi-debate` skill keeps them out of Git, and they are hash-checked from live bytes without entering Git coverage. Ignored untracked data is outside snapshot and guard coverage. Never force-add the whole worktree. Read the coverage contract in [references/phase_contracts.md](references/phase_contracts.md) before snapshotting.
